...
$basePath = '../../';
$pageTitle = 'Bugünkü Gebze - Gebze Belediyesi';
...
$stmt = $conn->query("SELECT icerik FROM sayfa_bugunku_gebze LIMIT 1");
$sayfaVerisi = $stmt->fetch(PDO::FETCH_ASSOC);
$sayfaIcerik = $sayfaVerisi ? $sayfaVerisi['icerik'] : '';
$stmt2 = $conn->query("SELECT * FROM bugunku_gebze_galeri ORDER BY sira ASC");
    ...
    <div class="container">
        <div class="kurumsal-grid kurumsal-grid-genis">
            ...
            <div class="gebze-yan-kolon">
                <?php $currentGebzePage = 'bugunku-gebze'; include '../../includes/gebze-sidebar.php'; ?>

                <div class="tarihce-galeri-dikey">
                    <h3 class="tarihce-galeri-baslik">Fotoğraflar</h3>
                    <?php foreach ($galeriFotolari as $index => $g): ?>
                        <img src="<?php echo $basePath; ?>includes/resim-goster.php?tablo=bugunku_gebze_galeri&id=<?php echo (int) $g['id']; ?>" alt="Bugünkü Gebze" loading="lazy" data-index="<?php echo $index; ?>">
                    <?php endforeach; ?>
                </div>
            </div>
        </div>
